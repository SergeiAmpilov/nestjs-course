...
import { API_URL, CLUSTER_SALARY_NOT_FOUND_ERROR, SALARY_CLUSTER_ID } from './hh.constants';
...
export class HhService {
  ...
  async getData(text: string) {
    try {
      const { data } = await firstValueFrom(this.httpService.get(API_URL.vacancies, {
        params: {
          text,
          cluster: true
        },
  ...
  private parseData(data: HhResponse): HHSalary {
    ...
    const middleSalary = this.getSalaryFromString(salaryCluster.items[
      Math.ceil(salaryCluster.items.length / 2)
    ].name);
